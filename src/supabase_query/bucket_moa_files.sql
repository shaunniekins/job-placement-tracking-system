-- add new bucket "moa-files" with mime type: "image/*,application/pdf"

-- add new policy for bucket "moa-files":
...
------- ((bucket_id = 'moa-files'::text) AND ((storage.foldername(name))[1] = 'public'::text) AND (auth.role() = 'authenticated'::text))

-- another: add new policy for bucket "moa-files":
---- Choose "Get started quickly"
